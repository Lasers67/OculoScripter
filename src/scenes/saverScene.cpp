...
void saverScene::setup(){
	...
	saveAndUpload.setup("save tag", 10, 10, 225, 100);
	saveAndUpload.setDisplayFont(&bigFont);
	saveAndUpload.setMaxCounter(buttonCount);

	saveAndUpload.setFlashLength(4.5);

	clearTags.setup("delete all tags", 10, 190, 225, 100);
	clearTags.setDisplayFont(&bigFont);
	clearTags.setMaxCounter(buttonCount*2);

	addTag.setup("add tag", 500, 20, 225, 100);
	addTag.setDisplayFont(&bigFont);
	addTag.setMaxCounter(buttonCount*2);

	clearLoaded.setup("clear loaded tags", 600, 20, 225, 100);
	clearLoaded.setDisplayFont(&bigFont);

	ofxXmlSettings xml;
	xml.loadFile("settings.xml");

	loadPrevious.setup("load prev tag", 10, 300, 225, 100);
	loadPrevious.setDisplayFont(&bigFont);
	loadPrevious.setRetrigger(true);
	loadNext.setup("load next tag", 10, 425, 225, 100);
	loadNext.setDisplayFont(&bigFont);
