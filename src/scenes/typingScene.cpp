...
void speakMe(const char * str){
	...
	{
		PlaySound(TEXT("sounds/food.wav"),NULL,SND_SYNC);
		//system("pause");
	}
	else if(strcmp(str,"water")==0)
	{
		PlaySound(TEXT("sounds/water.wav"),NULL,SND_SYNC);
		//system("pause");
	}
	else if(strcmp(str,"washroom")==0)
	{
		PlaySound(TEXT("sounds/washroom.wav"),NULL,SND_SYNC);
		//system("pause");
	}
	else if(strcmp(str,"sleep")==0)
	{
		PlaySound(TEXT("sounds/sleep.wav"),NULL,SND_SYNC);
		//system("pause");
	}
	else if(strcmp(str,"medicine")==0)
	{
		PlaySound(TEXT("sounds/medicine.wav"),NULL,SND_SYNC);
		//system("pause");
	}
	else if(strcmp(str,"impain")==0)
	{
		PlaySound(TEXT("sounds/impain.wav"),NULL,SND_SYNC);
		//system("pause");
	}
	else if(strcmp(str,"help")==0)
	{
		...
		for(int i=0;i<20;i++)
        {
            PlaySound(TEXT("sounds/beep.wav"),NULL,SND_SYNC);
        }
		//system("pause");
	}
}
...
void typingScene::setup(){
	...
	/*for (int i = 0; i < 6; i++){
		...
		nButton.setup(buttons[i], xStart, ofGetHeight()/3 + yStart, bWidth, bHeight);
    ...
    float xadd = 5;
	float yadd = 105;
    //float lowerMargin = letterButtons[5].getY(); //find y of last letter button
...
*/
	...
	clearButton.setup("CLEAR", (bWidth)+xadd, 500+(bHeight+xadd), bWidth*2+xadd, bHeight+40);
	clearButton.setMaxCounter(buttonCount);
	clearButton.setRetrigger(false);
	letterButtons.push_back(clearButton);

	buttonTrigger foodButton;
	foodButton.setup("Need Food!", (bWidth)+xadd,100+(bHeight+xadd), bWidth*2+xadd, bHeight+40);
	foodButton.setMaxCounter(buttonCount);
	foodButton.setRetrigger(false);
	letterButtons.push_back(foodButton);

	buttonTrigger waterButton;
	waterButton.setup("Water Please!", (bWidth*5)+xadd*5, 100+(bHeight+xadd), bWidth*2+xadd, bHeight+40);
	waterButton.setMaxCounter(buttonCount);
	waterButton.setRetrigger(false);
	letterButtons.push_back(waterButton);

	buttonTrigger washroomButton;
	washroomButton.setup("Washroom !!!", (bWidth*9)+xadd*9, 100+(bHeight+xadd), bWidth*2+xadd, bHeight+40);
	washroomButton.setMaxCounter(buttonCount);
	washroomButton.setRetrigger(false);
	letterButtons.push_back(washroomButton);

	buttonTrigger medButton;
	medButton.setup("Medicine", (bWidth*5)+xadd*5, 300+(bHeight+xadd), bWidth*2+xadd, bHeight+40);
	medButton.setMaxCounter(buttonCount);
	medButton.setRetrigger(false);
	letterButtons.push_back(medButton);

	buttonTrigger painButton;
	painButton.setup("I'm in Pain!", (bWidth*9)+xadd*9, 300+(bHeight+xadd), bWidth*2+xadd, bHeight+40);
	painButton.setMaxCounter(buttonCount);
	painButton.setRetrigger(false);
	letterButtons.push_back(painButton);

	buttonTrigger sleepButton;
	sleepButton.setup("I am gonna Sleep !", (bWidth)-xadd, 300+(bHeight+xadd), bWidth*2+xadd*6, bHeight+40);
	sleepButton.setMaxCounter(buttonCount);
	sleepButton.setRetrigger(false);
	letterButtons.push_back(sleepButton);

	buttonTrigger helpButton;
	helpButton.setup("HELP !!!", (bWidth*5)+xadd*5, 450+(bHeight+xadd), bWidth*6+xadd*4, bHeight*2);
	helpButton.setMaxCounter(buttonCount);
	helpButton.setRetrigger(false);
	letterButtons.push_back(helpButton);
...
//			}
			...
			else if (letterButtons[i].displayText == "CLEAR"){
				displayMessage.clear();
				carriageReturnCounter = 0;
			}
			else if (letterButtons[i].displayText == "Need Food!"){
				displayMessage.clear();
				for(unsigned int j=0;j<letterButtons[i].displayText.length();j++)
					displayMessage.push_back(letterButtons[i].displayText.at(j));
				speakMe("food");
				carriageReturnCounter = 0;
			}
			else if (letterButtons[i].displayText == "Water Please!"){
				displayMessage.clear();
				for(unsigned int j=0;j<letterButtons[i].displayText.length();j++)
					displayMessage.push_back(letterButtons[i].displayText.at(j));
				speakMe("water");
			...
			else if (letterButtons[i].displayText == "Washroom !!!"){
				displayMessage.clear();
				for(unsigned int j=0;j<letterButtons[i].displayText.length();j++)
					displayMessage.push_back(letterButtons[i].displayText.at(j));
				speakMe("washroom");
			...
			else if (letterButtons[i].displayText == "Medicine"){
				displayMessage.clear();
				for(unsigned int j=0;j<letterButtons[i].displayText.length();j++)
					displayMessage.push_back(letterButtons[i].displayText.at(j));
				speakMe("medicine");
			...
			else if (letterButtons[i].displayText == "I'm in Pain!"){
				displayMessage.clear();
				for(unsigned int j=0;j<letterButtons[i].displayText.length();j++)
					displayMessage.push_back(letterButtons[i].displayText.at(j));
				speakMe("impain");
			...
			else if (letterButtons[i].displayText == "I am gonna Sleep !"){
				displayMessage.clear();
				for(unsigned int j=0;j<letterButtons[i].displayText.length();j++)
					displayMessage.push_back(letterButtons[i].displayText.at(j));
				speakMe("sleep");
			...
			else if (letterButtons[i].displayText == "HELP !!!"){
				displayMessage.clear();
				for(unsigned int j=0;j<letterButtons[i].displayText.length();j++)
					displayMessage.push_back(letterButtons[i].displayText.at(j));
				speakMe("help");
