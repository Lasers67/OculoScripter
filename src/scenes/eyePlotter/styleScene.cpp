...
void styleScene::setup(){
	...
	selectAll.setup("Select All", 10, 10, 225, 80);
	selectAll.setDisplayFont(&bigFont);
	selectLetter.setup("Select Letter", 10, 110, 225, 80);
	selectLetter.setDisplayFont(&bigFont);
	strokeOptions.setup("Stroke Options", 10, 210, 225, 80);
	strokeOptions.setDisplayFont(&bigFont);
	fillOptions.setup("Fill Options", 10, 300, 225, 80);
	fillOptions.setDisplayFont(&bigFont);
	shadowOptions.setup("Shadow Options", 10, 390, 225, 80);
	shadowOptions.setDisplayFont(&bigFont);
	outlineOptions.setup("Outline Options", 10, 480, 225, 80);
	outlineOptions.setDisplayFont(&bigFont);
	...
	strokeDown.setup("< STROKE", 275, 20, 200, 100);
	strokeDown.setDisplayFont(&bigFont);
	//strokeDown.setMaxCounter(buttonCount * 2);
	strokeDown.setRetrigger(true);
	
	strokeUp.setup("STROKE > ", 500, 20, 200, 100);
	strokeUp.setDisplayFont(&bigFont);
	//strokeUp.setMaxCounter(buttonCount * 2);
	strokeUp.setRetrigger(true);
	
	brush.setup("BRUSH", 710, 20, 140, 100);
	brush.setDisplayFont(&bigFont);
	//brush.setMaxCounter(buttonCount * 2);
	brush.setRetrigger(true);
	
	toggleFill.setup("FILL","NO FILL", false, 275, 20, 150, 100);
	//toggleFill.setMaxCounter(buttonCount * 2);
	
	toggleHoles.setup("HOLES","NO HOLES", false, 450, 20, 150, 100);
	//toggleHoles.setMaxCounter(buttonCount * 2);
	
	lighting.setup("LIGHTING","NO LIGHTING", false, 800, 20, 140, 100);
	//lighting.setMaxCounter(buttonCount * 2);
	
	hashes.setup("HASH","NO HASH", false, 650, 20, 140, 100);
	//hashes.setMaxCounter(buttonCount * 2);

	shadowLeft.setup("< SHADOW", 275, 80, 150, 60);
	shadowLeft.setDisplayFont(&bigFont);
	//shadowLeft.setMaxCounter(buttonCount * 2);
	shadowLeft.setRetrigger(true);	
	
	shadowRight.setup("SHADOW >", 450, 80, 150, 60);
	shadowRight.setDisplayFont(&bigFont);
	//shadowRight.setMaxCounter(buttonCount * 2);
	shadowRight.setRetrigger(true);	
	
	shadowUp.setup("^ SHADOW", 350, 10, 160, 60);
	shadowUp.setDisplayFont(&bigFont);
	//shadowUp.setMaxCounter(buttonCount * 2);
	shadowUp.setRetrigger(true);	
	
	shadowDown.setup("v SHADOW", 350, 150, 160, 60);
	shadowDown.setDisplayFont(&bigFont);
	//shadowDown.setMaxCounter(buttonCount * 2);
	shadowDown.setRetrigger(true);	
	
	
	outlineDown.setup("< OUTLINE", 275, 20, 200, 100);
	outlineDown.setDisplayFont(&bigFont);
	//outlineDown.setMaxCounter(buttonCount * 2);
	outlineDown.setRetrigger(true);
	
	outlineUp.setup("OUTLINE > ", 500, 20, 200, 100);
	outlineUp.setDisplayFont(&bigFont);
	//outlineUp.setMaxCounter(buttonCount * 2);
	outlineUp.setRetrigger(true);
	
	pickColor.setup("COLOR","COLOR", false, 950, 20, 140, 100);
...
void styleScene::draw(){
	...
	if( state == STYLE_LETTER ){
		selectLeft.draw(255.0);
		selectRight.draw(255.0);
	}else if (state == STYLE_STROKE){
		strokeUp.draw(255.0);
		strokeDown.draw(255.0);
		pickColor.draw(255.0);
		brush.draw(255.0);
	}else if (state == STYLE_SHADOW){
		shadowLeft.draw(255.0);
		shadowRight.draw(255.0);
		shadowUp.draw(255.0);
		shadowDown.draw(255.0);
		pickColor.draw(255.0);
		lighting.draw(255.0);
		hashes.draw(255.0);
	}else if (state == STYLE_FILL){
		toggleFill.draw(255.0);
		toggleHoles.draw(255.0);
		pickColor.draw(255.0);
	}else if (state == STYLE_OUTLINE){
		outlineUp.draw(255.0);
		outlineDown.draw(255.0);
		pickColor.draw(255.0);
	}
